        ...
        fileTreeView.each(function (i, el) {
            $(el).off("dblclick").on("dblclick", function (event) {
                ...
                if (file_type != 'folder' && file_type != 'rootfolder') {
                    ...
                    $temp.remove();
                    editor.replaceSelection(file_path);
                }
            });
        });
        fileTreeView.each(function (i, el) {
...
// Plugin Settings
    ...
    function getCheckboxSessionsPreselected(preselect_session) {
        var groups = [];
        ...
        if (groups.length > 0) {
            getSessionsByGroups(groups, preselect_session);
        } else {
    ...
    function getSessionsByGroups(groups, preselect_session) {
        var csrf_token = $('input[name=csrfmiddlewaretoken]').val();
        $.ajax({
            url: base_url + '/admin/pages/get-sessions-by-groups/',
            dataType: "json",
            ...
            success: function (result) {
                if (result.error) {
                    ...
                    if (plugin_name == 'session-checkbox') {
                        ...
                        getCheckboxSessionsPreselected(preselect_session);
...
// Set plugin settings
    ...
    $('body').on('click', '.save-element-settings', function () {
        ...
        $this.closest('fieldset').find('.plugin-setting-multiple-registration-question-display').each(function () {
            var setting_id = $(this).find('ul:first').attr('data-setting-id');
            var selected_questions = attendee_treeview.getCheckedItems();
            var selected_questions_id = [];
            for (var i = 0; i < selected_questions.length; i++) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.error) {
                    ...
                    var element_settings = result.element_settings;
                    clog(element_settings);
                    showPluginUsingSettings(element_settings);
                    saveOrUpdate();
                }
