...
class CommonHelper(generic.DetailView):
    ...
    def get_datepicker_date_format(request):
        ...
        if date_format_obj.exists():
            format_json_obj = json.loads(date_format_obj[0].value)
            if 'datepicker' in format_json_obj:
                format = format_json_obj['datepicker']

        return format

    def get_moment_date_format(request):
        event_id = request.session['event_auth_user']['event_id']
        date_format_obj = Setting.objects.filter(event_id=event_id, name='default_date_format')
        format = 'YYYY-MM-DD'
        if date_format_obj.exists():
            format_json_obj = json.loads(date_format_obj[0].value)
            if 'moment' in format_json_obj:
                format = format_json_obj['moment']
    ...
    def get_timezone(request):
        event_id = request.session['event_auth_user']['event_id']
        timezone_obj = Setting.objects.filter(event_id=event_id, name='timezone')
        timezone = 'Europe/Stockholm'
        if timezone_obj.exists():
            timezone = timezone_obj[0].value

        return timezone

    def converStringToDate(date_str):
        return parse_date(date_str)

    def converStringToTime(time_str):
        return parse_time(time_str)

    def converStringToDateTime(datetime_str):
